...

{
  ...
  out_wd <- "Arrays_Constraint/"
}

speciesDisp <- suppressMessages(read_delim("data/species_meters.csv", delim = ";"))
...
if(!file.exists(glue::glue("{out_wd}/{sp}/binArray_Constraint.rda"))) {
  ...
  ### Sigmoidal dispersal
  sigm_params <- lapply(5, function(d) {
    lapply(1:3, function(x) {
      p50 <- as.numeric(spResults %>% pull(meters_year_50) %>% max()) * 30 * x * d
      p99 <- as.numeric(spResults %>% pull(meters_year_99) %>% max()) * 30 * x * d
  ...
  if(!file.exists(glue::glue("{out_wd}/{sp}/binArray_5TimesMaxDisp.rda"))) {
          ...
          p_dist <- distance %>% 
            mutate(runif = runif(prod(dim(.))),
                   p     = ifelse(distance<25, 1, 
                                  runif >= (1 / (1 + exp(-as.numeric(sigm_params[[1]][x,2])*(distance - as.numeric(sigm_params[[1]][x,1]))))))) %>%
      ...
      save(binarArray, file = glue::glue("{out_wd}/{sp}/binArray_Constraint.rda"))
      gc()
    }
